...
// /// - Start에서 RoomConfigSO(런타임 상태 포함)에 저장된 설정을 가져와 풍선 개수/시간 등을 매니저에 반영한다.
// /// </summary>
...
//     [SerializeField] private RoomConfigSO roomConfig; // 프리셋+런타임 상태 SO(씬 간 공유용)
...
//     {  
//         if (roomConfig == null)
//             Debug.LogWarning("MultiGameplayInitializer: RoomConfigSO가 할당되지 않았습니다. 기본값으로 진행합니다.", this);
    
//         RoomConfigDto cfg = roomConfig != null ? roomConfig.runtimeConfig : null; // 런타임 SO에서 설정 참조
//        // Debug.Log(cfg);
...
//         // GameManager에서 NetworkVariable 이벤트로 SetServerMatchStartTime을 호출해 갱신된다.
//         // 아래 임시 폴백은 테스트를 위해 주석 처리 (서버 시각 수신 실패 시 0으로 유지)
//         // ServerMatchStartTime = NetworkManager.Singleton != null ? (float)NetworkManager.Singleton.ServerTime.Time : 0f;
